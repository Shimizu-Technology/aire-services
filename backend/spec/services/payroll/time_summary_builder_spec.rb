...
RSpec.describe Payroll::TimeSummaryBuilder do
  describe "#call" do
    ...
    it "keeps pending work tracked without treating it as an included-hour blocker" do
      user = create(:user, first_name: "Pat", last_name: "Pending")
      category = create(:time_category, name: "Admin Duties")
      entry = create(
        :time_entry,
        user: user,
        time_category: category,
      ...
      expect(employee).to include(total_hours: 0.0, ready: true)
      expect(employee.dig(:issues, :pending_count)).to eq(1)
      expect(employee.fetch(:days).flat_map { |day| day.fetch(:entry_ids) }).not_to include(entry.id)
    end
  end
end
