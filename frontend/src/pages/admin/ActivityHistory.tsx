...
function EventDetail({ event, onClose }: { event: AuditLogEntry; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside role="dialog" aria-label="Activity event details" className="fixed inset-x-3 bottom-3 top-20 z-50 overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-xl xl:sticky xl:inset-auto xl:top-6 xl:z-auto xl:max-h-[calc(100vh-7rem)] xl:shadow-sm">
      <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-700">Event details</p>
...
export default function ActivityHistory() {
  ...
  const [filters, setFilters] = useState<AuditLogFilters>(() => {
    ...
    const subjectId = Number.parseInt(query.get('subject_id') || '', 10)
    return {
      page: 1,
      per_page: 50,
  ...
  const activeFilterCount = useMemo(() => [filters.event_category, filters.source, filters.from, filters.to, filters.outcome, filters.search, filters.subject_type && filters.subject_id].filter(Boolean).length, [filters])
  ...
  const clearSubjectScope = () => {
    setFilters((current) => ({ ...current, page: 1, subject_type: undefined, subject_id: undefined }))
    setSearchParams({}, { replace: true })
  }
  ...
  const exportEvents = async () => {
    setExporting(true)
    setError(null)
    const response = await api.downloadAuditLogs(filters)
    if (response.blob) {
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-cyan-700">Accountability</p>
        ...
        <button type="button" onClick={exportEvents} disabled={exporting || loading} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        {filters.subject_type && filters.subject_id && (
          <div className="mb-3 flex items-center justify-between gap-3 rounded-xl border border-cyan-100 bg-cyan-50 px-3 py-2 text-sm text-cyan-900">
            <span>Showing history for {formatLabel(filters.subject_type)} #{filters.subject_id}</span>
            <button type="button" onClick={clearSubjectScope} className="font-semibold text-cyan-700 hover:text-cyan-950">Show all</button>
          </div>
        )}
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          ...
          <select aria-label="Event category" value={filters.event_category || ''} onChange={(event) => updateFilter('event_category', event.target.value)} className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100">
            <option value="">All activity</option>
            {availableCategories.map((category) => <option key={category} value={category}>{formatLabel(category)}</option>)}
          </select>
          <select aria-label="Event source" value={filters.source || ''} onChange={(event) => updateFilter('source', event.target.value)} className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100">
            <option value="">All sources</option>
            {availableSources.map((source) => <option key={source} value={source}>{formatLabel(source)}</option>)}
          </select>
          <label className="relative"><span className="pointer-events-none absolute left-3 top-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">From</span><input aria-label="From date" type="date" value={filters.from || ''} onChange={(event) => updateFilter('from', event.target.value)} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 pt-3 text-sm text-slate-700 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100" /></label>
          <label className="relative"><span className="pointer-events-none absolute left-3 top-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">To</span><input aria-label="To date" type="date" value={filters.to || ''} onChange={(event) => updateFilter('to', event.target.value)} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 pt-3 text-sm text-slate-700 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100" /></label>
        </div>
        {activeFilterCount > 0 && <button type="button" onClick={clearFilters} className="mt-3 text-sm font-semibold text-cyan-700 transition hover:text-cyan-900">Clear {activeFilterCount} {activeFilterCount === 1 ? 'filter' : 'filters'}</button>}
      </section>
      ...
      <div className={`grid gap-6 ${selected ? 'xl:grid-cols-[minmax(0,1.35fr)_minmax(22rem,0.65fr)]' : ''}`}>
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
            ...
            {loading && <span className="text-xs font-medium text-cyan-700">Refreshing…</span>}
          </div>
          ...
          {loading && events.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100">
              {events.map((event) => (
                <button key={event.id} type="button" onClick={() => setSelected(event)} className={`grid w-full gap-3 px-4 py-4 text-left transition hover:bg-slate-50 sm:grid-cols-[8rem_minmax(0,1fr)_8rem] sm:items-center sm:px-5 ${selected?.id === event.id ? 'bg-cyan-50/70 ring-1 ring-inset ring-cyan-200' : ''}`}>
